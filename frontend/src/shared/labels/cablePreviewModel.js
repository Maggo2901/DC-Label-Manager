...
export function buildCableFirstRow(layoutSlug, raw = {}) {
  ...
  if (raw.rawInput) {
    const serial = String(raw.serialPrefix || '');
    ...
    return {
      ...
      lineName: lineId,
      // Rack/RU-Felder für layout-a-rack (leer wenn nicht eingetragen)
      rackA: String(raw.rackA || '').trim(),
      ruA:   String(raw.ruA   || '').trim(),
      rackB: String(raw.rackB || '').trim(),
      ruB:   String(raw.ruB   || '').trim(),
    };
  }
  ...
  if (config.layoutSlug === 'layout-a-rack') {
    return {
      aSide: config.aSideDevice,
      portA,
      zSide: config.zSideDevice,
      portB,
      rackA: String(raw.rackA || '').trim(),
      ruA:   String(raw.ruA   || '').trim(),
      rackB: String(raw.rackB || '').trim(),
