...
const MODE_BADGE = {
  ...
  tape: 'bg-fuchsia-900/60 text-fuchsia-200 border-fuchsia-800/50',
};

function normalizeHistoryMode(value) {
  const mode = String(value || '').toLowerCase();
  if (mode === 'single' || mode === 'batch' || mode === 'excel' || mode === 'tape') {
    return mode;
  }
  return 'single';
}

/** Detect whether a print job is from the P-Touch module */
function isPtouchJob(job) {
  return job?.module_key === 'ptouch' || job?.mode === 'tape';
}
         ...
         {selectedJob ? (
            <div className="flex flex-col h-full">
                ...
                <div className="flex-1 overflow-y-auto p-6 min-h-0">
                   {isPtouchJob(selectedJob) ? (
                     /* ── P-Touch label data ── */
                     <>
                       <h4 className="text-sm font-bold text-slate-300 mb-3 flex items-center gap-2">
                          <FileText className="h-4 w-4" />
                          Label Data Preview ({selectedJob.payload?.labels?.length || 0})
                       </h4>
                       {selectedJob.payload?.tapeWidth && (
                         <p className="text-xs text-slate-500 mb-3">
                           Tape: {selectedJob.payload.tapeWidth}mm
                           {selectedJob.payload.duplicateCount > 1 && ` · ${selectedJob.payload.duplicateCount}× duplicates`}
                         </p>
                       )}
                       <div className="space-y-2 overflow-y-auto custom-scrollbar pr-2">
                          {(selectedJob.payload?.labels || []).map((label, idx) => (
                             <div key={idx} className="flex items-center gap-4 p-2 rounded border border-slate-700 bg-slate-800/50 text-xs font-mono text-slate-300">
                       ...
                       <div className="flex items-center justify-between p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/30">
                          ...
                          <button
                              onClick={() => {
                                  ...
                                  if (isPtouchJob(selectedJob)) {
                                    // P-Touch restore: save labels as ptouch draft
                                    const ptouchPayload = {
                                      labels: (selectedJob.payload.labels || []).map((text, i) => ({
                                        text,
                                        creationSource: 'manual',
                                        tapeWidth: selectedJob.payload.tapeWidth || 12,
                                        createdAt: Date.now(),
                                        repeatCount: 1,
                                      })),
                                      config: {
                                        tapeWidth: selectedJob.payload.tapeWidth || 12,
                                        duplicateEnabled: (selectedJob.payload.duplicateCount || 1) > 1,
                                        duplicateCount: selectedJob.payload.duplicateCount || 2,
                                      },
                                    };
                                    draftsApi.save('ptouch', ptouchPayload)
                                      .then(() => { window.location.hash = '#/ptouch'; })
                                      .catch(err => {
