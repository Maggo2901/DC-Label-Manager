...
export default function CableInputColumn({
  ...
  return (
    <div className="flex h-full flex-col gap-3">
      {/* ── Top Card: Inputs ── */}
      <div className="dc-panel flex-1 min-h-0 overflow-y-auto custom-scrollbar">
        <div className="flex flex-col gap-3 p-4">
          ...
          {mode === 'single' && (
            <div className="flex flex-col gap-3">
              ...
              {/* Rack/RU-Felder — nur sichtbar wenn layout-a-rack gewählt */}
              {config.layoutSlug === 'layout-a-rack' && (
                <>
                  <hr className="border-slate-700" />
                  <div>
                    <label className="dc-section-label mb-2 block">Rack / RU — Seite A</label>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="dc-label">Rack A</label>
                        <input
                          value={config.rackA || ''}
                          onChange={(e) => handleChange('rackA', e.target.value)}
                          placeholder="z.B. 042"
                          className="dc-input"
                        />
                      </div>
                      <div>
                        <label className="dc-label">RU A</label>
                        <input
                          value={config.ruA || ''}
                  ...
                  <div>
                    <label className="dc-section-label mb-2 block">Rack / RU — Seite B</label>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="dc-label">Rack B</label>
                        <input
                          value={config.rackB || ''}
                          onChange={(e) => handleChange('rackB', e.target.value)}
                          placeholder="z.B. 043"
                          className="dc-input"
                        />
                      </div>
                      <div>
                        <label className="dc-label">RU B</label>
                        <input
                          value={config.ruB || ''}
                          onChange={(e) => handleChange('ruB', e.target.value)}
                          placeholder="z.B. 01"
                          className="dc-input"
                        />
                      </div>
              ...
              <div>
                ...
                <input
                    ...
                    placeholder="z.B. Line ID, Notizen"
                    className="dc-input"
                  />
              </div>
            </div>
      ...
      {/* ── Bottom Card: Action ── */}
      <div className="dc-panel shrink-0 h-[60px] flex items-center p-3">
        {mode === 'single' && (
