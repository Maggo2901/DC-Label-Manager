      ...
      try {
        ...
        const processChunk = () => {
             ...
             for (let j = i; j < end; j++) {
                 ...
                 // Felder auf das interne Datenmodell abbilden
                 const item = {
                     ...
                     // Optionale Rack/RU-Felder fuer layout-a-rack
                     rackA: String(row.rackA || '').trim(),
                     ruA:   String(row.ruA   || '').trim(),
                     rackB: String(row.rackB || '').trim(),
                     ruB:   String(row.ruB   || '').trim(),
