...
function sanitizeRowForLayout(row, layoutSlug) {
  ...
  if (layoutSlug === 'layout-a-rack') {
    return {
      aSide: String(source.aSide ?? ''),
      portA: String(source.portA ?? ''),
      zSide: String(source.zSide ?? ''),
      portB: String(source.portB ?? ''),
      rackA: String(source.rackA ?? '').trim(),
      ruA:   String(source.ruA   ?? '').trim(),
      rackB: String(source.rackB ?? '').trim(),
      ruB:   String(source.ruB   ?? '').trim(),
    };
  }
...
const CableLabelsModule = () => {
  ...
  const generatePreviewFromConfig = () => {
    ...
    return {
       ...
       // Rack/RU-Felder für layout-a-rack (aus Config übernehmen)
       rackA: String(config.rackA || '').trim(),
       ruA:   String(config.ruA   || '').trim(),
       rackB: String(config.rackB || '').trim(),
       ruB:   String(config.ruB   || '').trim(),
    };
  };
  ...
  return (
    <ModulePageLayout>
      ...
      <ModulePageLayout.Content className="p-4">
      <div className="grid h-full min-h-0 gap-4 lg:grid-cols-[360px_1fr_360px]">
        ...
        <div className="min-h-0 flex flex-col overflow-hidden">
          ...
          {mode === 'excel' && (
            <div className="dc-panel flex-1 min-h-0 overflow-hidden">
              <CableExcelInput 
                  config={config}
                  setConfig={setConfig}
                  onImport={(rows) => {
        ...
        <div className="flex min-h-0 flex-col gap-3 overflow-hidden">
          ...
          <div className="shrink-0">
              <div className="flex items-center gap-3 dc-panel h-[60px] p-3">
                 <button 
