...
/**
 * Berechnet die kompakte Rack/RU-Darstellung aus den Rohdaten einer Seite.
 *
 * Regeln:
 *  - rack + ru vorhanden  → 'R{rack}/RU{ru}'
 *  - nur rack vorhanden   → 'R{rack}'
 *  - nur ru vorhanden     → 'RU{ru}'
 *  - beides leer          → '' (Element wird nicht gerendert)
 *
 * @param {string} rackKey  – Daten-Key für den Rack-Wert
 * @param {string} ruKey    – Daten-Key für den RU-Wert
 * @param {Object} row      – Label-Datensatz
 * @returns {string}
 */
function resolveRackRuPair(rackKey, ruKey, row) {
  const rack = String(row[rackKey] ?? '').trim();
  const ru   = String(row[ruKey]   ?? '').trim();

  if (rack && ru)  return 'R' + rack + '/RU' + ru;
  if (rack)        return 'R' + rack;
  if (ru)          return 'RU' + ru;
  return '';
}

/**
 * Löst computedKey-Bezeichner auf, die nicht direkt einem einzelnen
 * Datenfeld entsprechen, sondern aus mehreren Feldern zusammengesetzt sind.
 *
 * Aktuell unterstützte computedKey-Werte:
 *  - 'rackRuA' → Rack-/RU-Kombination für Seite A
 *  - 'rackRuB' → Rack-/RU-Kombination für Seite B
 */
function resolveComputedKey(computedKey, row) {
  if (computedKey === 'rackRuA') return resolveRackRuPair('rackA', 'ruA', row);
  if (computedKey === 'rackRuB') return resolveRackRuPair('rackB', 'ruB', row);
  return '';
}
 ...
 *  2. `computedKey`  – dynamisch berechneter Wert (z.B. Rack/RU-Kombination)
 *  3. `resolveKeys`  – first non-empty value from an ordered key list
 *  4. `key`          – single data key
 *
...
export function resolveText(row, element) {
  ...
  if (element.computedKey) {
    return resolveComputedKey(element.computedKey, row);
  }
